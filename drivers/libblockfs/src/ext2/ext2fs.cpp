...
async::result<void> FileSystem::init() {
	...
	for(size_t offset = 0; offset < bgdtSize; offset += blockSize)
		bgdtBlocks.push_back(co_await accessMetadata(bgdtBlock + (offset >> blockShift), false));
	bgdt.init(std::move(bgdtBlocks), blockSize, blockGroupDescriptorSize);
...
FileSystem::synchronize(std::shared_ptr<Inode> inode,
	...
	co_await synchronizeMutex.async_lock();
	frg::unique_lock synchronizeLock{frg::adopt_lock, synchronizeMutex};
